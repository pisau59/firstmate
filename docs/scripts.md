...
| `fm-spawn.sh`            | Spawn one task, several `id=repo` pairs, or a persistent secondmate with `--secondmate`; ship/scout spawns require an isolated treehouse worktree; secondmate spawns locally sync the home before launch; pi crewmates are pinned to `--model opencode-go/deepseek-v4-flash` while secondmates keep the default model |
